...
**Updated:** 2026-09-22. **Phase:** 1, working database and backend layer. **Step:** migrations applied locally, invariant tests next.
...
- Local Supabase stack running under Colima with analytics off; `.env.local` in place. `drizzle/migrations/0000_init.sql` and `0001_triggers.sql` generated, reviewed against DATA_MODEL.md, applied locally, and the nine SETUP.md section 5 checks pass by hand. On branch `p1/migrations`.
...
- Git: `main` on GitHub at rafipinc/dealZ with the phase 0 and scaffold commits, CI green.

## What does not exist yet

No tests, no services, no `src/lib`. The migrations branch is not yet committed.

## Next

Phase 1, in this order, one per session unless small:

1. Commit and push `p1/migrations`, open the pull request, see CI green, merge.
2. `src/db/invariants.test.ts` against PGlite: the full matrix in TESTING.md.
3. `src/lib`: gtin, url, model-code parsing, with unit tests.
4. Services `retailers`, `catalog`, `listings`, `observations`, `deals`, each with its test file.
